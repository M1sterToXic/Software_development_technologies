...
void test_dsymm(int m, int n, int num_threads, int parallel) {
    omp_set_num_threads(num_threads);
    openblas_set_num_threads(num_threads);
    ...
    for (int rep = 0; rep < 10; ++rep) {
        ...
        memset(C_ref, 0, ldc * n * sizeof(double));
        t1 = get_time();
...
int main() {
    ...
    printf("\n------------ double (dsymm) - последовательная версия ------------\n");
    for (int i = 0; i < nthreads; ++i) {
        test_dsymm(m, n, threads[i], 0);
    }
    printf("\n------------ double (dsymm) - параллельная версия (OpenMP) ------------\n");
    for (int i = 0; i < nthreads; ++i) {
        test_dsymm(m, n, threads[i], 1);
    }
